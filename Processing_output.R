...
spplot(prob, zcol = "Variance", main = "Variance of flooding", col.regions = colorRampPalette(c("white", "green", "yellow", "red"))(30), 
       sp.layout = list("sp.polygons", studarea, first = F))
spplot(prob, zcol = "sd", main = "Sdv of flooding", col.regions = colorRampPalette(c("white", "green", "yellow", "red"))(30), 
       sp.layout = list("sp.polygons", studarea, first = F))
...
# Read all flooding model results from second experiment into a matrix "exp2"
...
# plot using blue, pink, yellow colorramp
spplot(prob2, zcol=1, col.regions=bpy.colors())
